...
export const Libs: FC<ComponentI18n> = ({ translate, loading }) => (
  <section className="w-full py-28 bg-paper-light">
    <header className="w-full px-4 lg:px-1 m-auto max-w-7xl">
      <Chip className="text-primary bg-primary-light">
        {loading ? (
          <Skeleton width="120px" height="10px" />
        ) : (
          translate("section_libs_chip")
        )}
      </Chip>
      ...
      <h3 className="font-poppins max-w-[741px] text-neutral text-lg lg:text-xl mt-4">
        {loading ? (
          ...
          translate("section_libs_description")
        )}
      </h3>
    </header>
    <section className="px-4 lg:px-0 my-12 m-auto max-w-7xl">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {libs.map((lib) => (
          <section
            key={lib.name}
            className="bg-paper flex flex-col gap-3 rounded-lg border border-solid border-neutral-light py-11 px-10"
          >
            <div className="flex flex-row justify-between">
              ...
              >
                {loading ? (
                  ...
                  translate("section_libs_button")
                )}
                <span className="sr-only"> - {lib.name}</span> <GitHubIcon />
              </a>
            </div>
            <p className="text-neutral">
              {loading ? <Skeleton width="100%" /> : translate(lib.description)}
            </p>
            <a
              href={lib.githubLink}
              target="_blank"
      ...
      <div className="w-fit m-auto mt-12 lg:mt-24">
        <ButtonLink external to={AppLinks.github} outline>
          {loading ? (
            ...
            translate("section_libs_github_button")
          )}
